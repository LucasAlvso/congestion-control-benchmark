...
docker exec -d tcp-client1 /bin/sh -c "tc qdisc add dev eth0 root netem delay 50ms 20ms distribution normal && echo 'put test-files/test_200MB.bin' | ./client --host=server --port=8080 --log-dir=./logs"
docker exec -d tcp-client2 /bin/sh -c "tc qdisc add dev eth0 root netem delay 50ms 20ms distribution normal && echo 'put test-files/test_200MB.bin' | ./client --host=server --port=8080 --log-dir=./logs"
docker exec -d tcp-client3 /bin/sh -c "tc qdisc add dev eth0 root netem delay 50ms 20ms distribution normal && echo 'put test-files/test_200MB.bin' | ./client --host=server --port=8080 --log-dir=./logs"
