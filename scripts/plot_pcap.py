...
def plot_time_sequence(df, png_path):
    ...
    if not mask.any():
        print("No valid seq packets to plot time-sequence.")
        return
...
def plot_bytes_in_flight(df, png_path):
    ...
    if not mask.any():
        print("No valid bytes_in_flight samples to plot.")
        return
...
def main():
    ...
    # Ensure expected columns exist (tshark may produce headers with slightly different names)
    # Normalize header names to exact expected strings if possible
    ...
    # If we found alternative names, rename columns to expected names
    if mapping:
